...
module.exports = {
      ...
      if (has(req.body, field)) {
        if (field === 'password') {
          ...
          if(!(req.body[field] === 'true' || req.body[field] === 'false'))
            throw new CodeError('isAdmin is not BOOLEAN', status.FORBIDDEN)
          else
  ...
  async updatePassword(req, res) {
    ...
    if (has(req.body, 'password')) {
        console.log("Password found in request.");
        ...
        throw new CodeError('No password!', status.FORBIDDEN);
    }

    console.log("Updating user password in database for ID:", req.user.id);
    await userModel.update(userModified, { where: { id: req.user.id } });
